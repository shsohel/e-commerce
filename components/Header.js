...
const Header = () => {
  ...
  const handleMenuCollapse = (menuId, authButtonId) => {
    ...
    const findClassList =Object.values(menu?.classList).find(s=> s === "hidden");
    ...
    <div>
      <nav className="flex items-center justify-between flex-wrap bg-neutral-100 px-4 p-2">
        ...
        >
          ...
          <VscGlobe  size={30} className=" hidden lg:inline-block  text-slate-800 hover:text-slate-600  mr-3  "/>
            <a
              href="#"
              className="block lg:inline-block  text-sm px-4 py-[7px] leading-none border border-neutral-800 rounded text-slate-800 hover:text-slate-600 hover:border-transparent hover:bg-white mt-4 lg:mt-0"
            >
              Login
            </a>
            <a
